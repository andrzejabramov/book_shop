...
function init() {
  ...
  const cached = localStorage.getItem("book_categories");
  STATE.categories = cached
    ? JSON.parse(cached)
    : [...new Set(localBooks.map((b) => b.category))];
  localStorage.setItem("book_categories", JSON.stringify(STATE.categories));
...
function renderCategoryList() {
  ...
  if (!list) {
    console.error("❌ .category-list не найдена!");
    return;
  }

  // Рендер списка с новыми классами
  ...
  // 🔥 Прямой обработчик на каждую ссылку
...
function loadPage(resetGrid = false) {
  const grid = document.getElementById("card-grid");
  if (!grid || !STATE.template) return;

  const filtered = localBooks.filter(
    (b) => b.category === STATE.activeCategory,
  );
  const page = filtered.slice(STATE.offset, STATE.offset + STATE.limit);

  // ✅ Безопасная очистка: удаляем только карточки, шаблон остаётся в DOM
  if (resetGrid) {
  ...
  page.forEach((book) => {
    ...
    // ✅ title уже обрезан в books-local.js, CSS подстрахует визуально
...
function attachButtons() {
  document
    .querySelectorAll(".card__btn.load-more-btn:not([data-attached])")
    .forEach((btn) => {
